...
def plot_histograms(prefijo, carpeta_entrada, carpeta_salida):
    ...
    for tipo in tipos:
        ...
        for archivo in archivos:
            ...
            # Definir un array de posiciones entre -5 y 5, equiespaciadas
            x = np.linspace(-5, 5, num=100)  # puedes ajustar el número de puntos

            # Calcular la curva exponencial exp(-0.5 * x^2)
            exp_curve = np.exp(-0.5 * x**2)/np.sqrt(2*np.pi)


            # crear figura
            plt.figure(figsize=(8,5))
            plt.bar(x_vals, y_vals, width=widths, align='center', color='skyblue', edgecolor='black')
            plt.plot(x, exp_curve, 'r-', linewidth=2, label='exp(-E) normalizada')
            plt.legend()
            plt.xlabel('x')
